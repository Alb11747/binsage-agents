...
Use `pool_schedule_plan` to obtain the saved revision, schedule IDs, and desired changes. Apply those changes through the native scheduling tool, then call `pool_schedule_save` with that revision and the retained IDs. The client does not create an OS scheduler. Anonymous support is available through `support-anonymous --args` even if authenticated identity is broken.
