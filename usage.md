...
Compute an earlier wake from the next quota threshold transition or expected useful work, so a twelve-hour baseline does not sleep through the final-six-hour or final-hour opportunity. By default, keep the task running and use native `clock.sleep` until that time, or `pool_wait` when queue/mailbox events should wake it. Extend the current run's expected wake before sleeping, or finish the cycle and begin a new one after wake. If an earlier independent native schedule is demonstrably available, reconcile its saved ID rather than accumulating schedules. Keep the permanent baseline throughout. Acquire the server's account contribution session before work so overlapping wakes or a second computer cannot duplicate contribution.
